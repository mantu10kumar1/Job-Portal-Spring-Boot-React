    ...
    <div className=" w-4/5 mx-auto ">
      ...
      <div className="px-3 mt-16 ">
        <div className="text-3xl font-semibold flex justify-between ">
          {" "}
          {props.name}
          <ActionIcon size="lg" variant="subtle" color="brightSun.4" onClick={() => handleEdit(0)}>
           {edit[0]? <IconDeviceFloppy className=" h-4/5 w-4/5 " /> : <IconPencil
      ...
      <div className="px-3 ">
        ...
        <div className="flex flex-wrap gap-2 ">
          {props.skills.map((skill: any, index: any) => (
            <div
              key={index}
              className="bg-bright-sun-300  bg-opacity-15 text-sm font-medium rounded-3xl text-bright-sun-400 px-3 py-1 "
            >
              {skill}
            </div>
          ))}
